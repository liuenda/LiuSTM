...
class LSTM():    
    def __init__(self, nam, load=False, training=False):
        ...
        if training == True:
            ...
            gradi = tensor.grad(cost, wrt = tnewp.values()) # T.grad -> differential
            grads = []
            l = len(gradi)
    ...
    def train_lstm(self, train, max_epochs):
        ...
        for eidx in xrange(0,max_epochs):
            sta = time.time()
            print self.chkterr2(test) # not shown in the orgin version

            num = len(train) # length of 
            # nd = eidx
            print 'Epoch', eidx
            # rnd = sample(xrange(len(train)), len(train))
            rnd=sample(xrange(num), num) # generate a list with length of num from the population xrange(num)
